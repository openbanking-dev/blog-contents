...
*If you are familiar with OAuth2 and don't know the hybrid flow yet: the hybrid is very similar to the authorisation code grant flow, it just returns an ID token with the authorisation code.*
...
OIDC is like a layer on top of OAuth2. For Open Banking, We are not really going to use the identity layer added by OIDC, but just use it for additional security check it provides. We will come back on that.
...
* `aud`: like when you write a letter, you are going to say who is the audience of this message. In this case, it is the bank. To find what is the official audience value corresponding to the bank, remember about the AS discovery. In there, the bank told us their `issuer` value. That this value we are going to use.
